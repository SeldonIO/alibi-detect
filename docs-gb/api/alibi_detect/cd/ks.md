...
| `p_val` | `float` | `0.05` | p-value used for significance of the K-S test for each feature. If the FDR correction method is used, this corresponds to the acceptable q-value. |
...
| `alternative` | `str` | `'two-sided'` | Defines the alternative hypothesis. Options are 'two-sided', 'less' or 'greater'. |
| `n_features` | `Optional[int]` | `None` | Number of features used in the K-S test. No need to pass it if no preprocessing takes place. In case of a preprocessing step, this can also be inferred automatically but could be more expensive to compute. |
