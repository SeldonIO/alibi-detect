...
* `backend`: **TensorFlow**, **PyTorch** and [**KeOps**](https://github.com/getkeops/keops) implementations of the MMD detector are available. Specify the backend (_tensorflow_, _pytorch_ or _keops_). Defaults to _tensorflow_.
...
* `configure_kernel_from_x_ref`: If `sigma` is not specified, the detector can infer it via a heuristic and set `sigma` to the median (_TensorFlow_ and _PyTorch_) or the mean pairwise distance between 2 samples (_KeOps_) by default. If `configure_kernel_from_x_ref` is _True_, we can already set `sigma` at initialization of the detector by inferring it from `x_ref`, speeding up the prediction step. If set to _False_, `sigma` is computed separately for each test batch at prediction time.
...
We can also easily add preprocessing functions for the _TensorFlow_ and _PyTorch_ frameworks. Note that we can also combine for instance a PyTorch preprocessing step with a KeOps detector. The following example uses a randomly initialized image encoder in PyTorch:
...
We detect data drift by simply calling `predict` on a batch of instances `x`. We can return the p-value and the threshold of the permutation test by setting `return_p_val` to _True_ and the maximum mean discrepancy metric and threshold by setting `return_distance` to _True_.
