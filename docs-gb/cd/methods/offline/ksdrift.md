...
* `alternative`: Defines the alternative hypothesis. Options are _'two-sided'_ (default), _'less'_ or _'greater'_.
...
* `threshold`: for feature-level drift detection the threshold equals the p-value used for the significance of the K-S test. Otherwise the threshold after the multivariate correction (either _bonferroni_ or _fdr_) is returned.
* `distance`: feature-wise K-S statistics between the reference data and the new batch if `return_distance` equals _True_.
