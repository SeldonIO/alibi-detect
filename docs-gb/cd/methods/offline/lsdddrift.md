...
$LSDD$ is based on the assumption that a probability density exists for both distributions and hence is only suitable for continuous data. If you are working with tabular data containing categorical variables, we recommend using the [TabularDrift detector](../tabulardrift.ipynb) instead.
...
* `backend`: Both **TensorFlow** and **PyTorch** implementations of the LSD detector as well as various preprocessing steps are available. Specify the backend (_tensorflow_ or _pytorch_). Defaults to _tensorflow_.
...
* `distance`: LSDD metric between the reference data and the new batch if `return_distance` equals _True_.
