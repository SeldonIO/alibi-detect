...
* `ert`: The expected run-time in the absence of drift, starting from _t=0_.
...
* `n_kernel_centers`: The number of reference samples to use as centers in the Gaussian kernel model used to estimate LSDD. Defaults to 2\*window\_size.
...
* `test_stat`: LSDD metric between the reference data and the test\_window if `return_test_stat` equals _True_.
* `threshold`: The value the test-statsitic is required to exceed for drift to be detected if `return_test_stat` equals _True_.
