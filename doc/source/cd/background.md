...
If a ground truth is available at test time,
...
and the null $P(\mathbf{z})=P_{ref}(\mathbf{z})$ is rejected, when
...
The univariate tests assume each test (i.e. each feature dimension in this case) 
are independent. Usually this is not the case, resulting in false positive rates 
(FPR's) up to $d$-times lower than desired, which becomes especially problematic 
when $d$ is large. The multivariate tests, which use permutation testing to extract
$p$-values, allow us to keep our false positive rates known, at the cost of greater
complexity.
...
The paper [Failing Loudly: An Empirical Study of
...
The 2D input data is generated by sampling from a mixture of two Normal distributions:
...
$k(\mathbf{z},\mathbf{z}')$ to compute unbiased estimates of
...
$$
k(\mathbf{z},\mathbf{z}') = \left(1-\epsilon\right)\, k_a\!\left(\Phi(\mathbf{z}),\Phi(\mathbf{z}') \right) + \epsilon \, k_b\!\left(\mathbf{x},\mathbf{x}'\right),
$$
