...
start_clip = [10, 1000]
max_n = [None, 50]
threshold_perc = [75.0, 95.0]
return_instance_score = [True, False]

tests = list(product(threshold, n_components, std_clip, start_clip, max_n, threshold_perc, return_instance_score))
n_tests = len(tests)
...
def test_mahalanobis(mahalanobis_params):
    threshold, n_components, std_clip, start_clip, max_n, threshold_perc, return_instance_score = mahalanobis_params
    X, y = load_iris(return_X_y=True)
    mh = Mahalanobis(threshold, n_components=n_components, std_clip=std_clip, start_clip=start_clip, max_n=max_n)
    ...
    if return_instance_score:
        assert (
            ...
            == (od_preds["data"]["instance_score"] > mh.threshold).astype(int).sum()
        )
    else:
