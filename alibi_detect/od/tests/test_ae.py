...
tests = list(
    product(threshold, threshold_perc, return_instance_score, return_feature_score, outlier_perc, outlier_type)
)
n_tests = len(tests)
...
def test_ae(ae_params):
    ...
    # init OutlierAE
    ae = OutlierAE(threshold=threshold, encoder_net=encoder_net, decoder_net=decoder_net)
    ...
    if outlier_type == "instance":
        ...
        if return_instance_score:
            assert (
                ...
                == (od_preds["data"]["instance_score"] > ae.threshold).astype(int).sum()
            )
        ...
        if return_feature_score:
            assert (
                od_preds["data"]["is_outlier"].sum()
                == (od_preds["data"]["feature_score"] > ae.threshold).astype(int).sum()
            )
