...
d_fit = {"ds": pd.date_range(pd.datetime.today(), periods=100), "y": np.random.randn(100)}
df_fit = pd.DataFrame(data=d_fit)

d_test = {"ds": pd.date_range(d_fit["ds"][-1] + datetime.timedelta(1), periods=100), "y": np.random.randn(100)}
...
def test_prophet(prophet_params):
    ...
    od_preds = od.predict(df_test, return_instance_score=return_instance_score, return_forecast=return_forecast)
