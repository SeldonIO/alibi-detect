...
from alibi_detect.saving._pytorch import (get_pt_dtype, load_embedding_pt, load_kernel_config_pt, load_model_pt,
                                          load_optimizer_pt, prep_model_and_emb_pt)
from alibi_detect.saving._sklearn import load_model_sk
from alibi_detect.saving._tensorflow import (get_tf_dtype, load_detector_legacy, load_embedding_tf,
                                             load_kernel_config_tf, load_model_tf, load_optimizer_tf,
...
def _load_preprocess_config(cfg: dict) -> Optional[Callable]:
    """
    Builds a preprocess_fn from the preprocess dict in a detector config dict.

    The dict format is expected to match that generated by serialize_preprocess in
    alibi_detect.utils.saving (also see pydantic schema). The model, tokenizer and
...
def read_config(filepath: Union[os.PathLike, str]) -> dict:
    """Reads a detector toml config file and returns a dict specifying the detector.
