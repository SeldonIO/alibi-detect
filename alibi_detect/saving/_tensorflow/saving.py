...
def save_model_config(
    model: Callable, base_path: Path, input_shape: Optional[tuple], local_path: Path = Path(".")
    ...
    if model is not None:
        ...
        cfg_model = {"flavour": Framework.TENSORFLOW.value, "src": local_path.joinpath("model")}
...
def save_model(
    ...
    filename: str = "model",
    save_format: Literal["tf", "h5"] = "h5",
) -> None:  # TODO - change to tf, later PR
    """
...
#######################################################################################################
def save_embedding_legacy(embed: TransformerEmbedding, embed_args: dict, filepath: Path) -> None:
    """
    Save embeddings for text drift models.
...
def preprocess_step_drift(
    cd: Union[ChiSquareDrift, ClassifierDriftTF, KSDrift, MMDDriftTF, TabularDrift]
) -> Tuple[
    Optional[Callable], Dict, Optional[tf.keras.Model], Optional[TransformerEmbedding], Dict, Optional[Callable], bool
]:
...
def state_chisquaredrift(
    cd: ChiSquareDrift,
) -> Tuple[Dict, Optional[tf.keras.Model], Optional[TransformerEmbedding], Optional[Dict], Optional[Callable]]:
    """
    ...
    """
    preprocess_fn, preprocess_kwargs, model, embed, embed_args, tokenizer, load_emb = preprocess_step_drift(cd)
    state_dict = {
        "args": {"x_ref": cd.x_ref},
        "kwargs": {
            "p_val": cd.p_val,
...
def state_classifierdrift(
    cd: ClassifierDrift,
) -> Tuple[
    Dict, tf.keras.Model, Optional[tf.keras.Model], Optional[TransformerEmbedding], Optional[Dict], Optional[Callable]
]:
    """
    ...
    cd
        Drift detection object.
    """
    preprocess_fn, preprocess_kwargs, model, embed, embed_args, tokenizer, load_emb = preprocess_step_drift(
        cd._detector
    )
...
def state_iforest(od: IForest) -> Dict:
    ...
    """
    state_dict = {"threshold": od.threshold, "isolationforest": od.isolationforest}
    return state_dict
...
def state_adv_md(md: ModelDistillation) -> Dict:
    ...
    """
    state_dict = {"threshold": md.threshold, "temperature": md.temperature, "loss_type": md.loss_type}
    return state_dict
...
def state_prophet(od: OutlierProphet) -> Dict:
    ...
    od
        Outlier detector object.
    """
    state_dict = {"model": od.model, "cap": od.cap}
    return state_dict
...
def save_tf_ae(detector: Union[OutlierAE, AdversarialAE], filepath: Union[str, os.PathLike]) -> None:
    """
...
def save_tf_vae(detector: OutlierVAE, filepath: Union[str, os.PathLike]) -> None:
    """
...
def save_tf_hl(models: List[tf.keras.Model], filepath: Union[str, os.PathLike]) -> None:
    """
...
def save_tf_aegmm(od: OutlierAEGMM, filepath: Union[str, os.PathLike]) -> None:
    """
...
def save_tf_vaegmm(od: OutlierVAEGMM, filepath: Union[str, os.PathLike]) -> None:
    """
...
def save_tf_s2s(od: OutlierSeq2Seq, filepath: Union[str, os.PathLike]) -> None:
    """
