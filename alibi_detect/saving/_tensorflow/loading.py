...
def load_detector_legacy(filepath: Union[str, os.PathLike], suffix: str, **kwargs) -> Detector:
    ...
    try:
        if meta_dict["version"] != __version__:
            warnings.warn(
                f'Trying to load detector from version {meta_dict["version"]} when using version '
...
def load_tf_vae(filepath: Union[str, os.PathLike], state_dict: Dict) -> tf.keras.Model:
    """
    Load VAE.
...
def load_tf_aegmm(filepath: Union[str, os.PathLike], state_dict: Dict) -> tf.keras.Model:
    """
    Load AEGMM.
...
def load_tf_vaegmm(filepath: Union[str, os.PathLike], state_dict: Dict) -> tf.keras.Model:
    """
    Load VAEGMM.
...
def load_tf_s2s(filepath: Union[str, os.PathLike], state_dict: Dict) -> tf.keras.Model:
    """
...
def load_tf_llr(
    filepath: Union[str, os.PathLike],
    dist_s: Union[Distribution, PixelCNN] = None,
    dist_b: Union[Distribution, PixelCNN] = None,
...
def init_od_ae(state_dict: Dict, ae: tf.keras.Model) -> OutlierAE:
    """
...
def init_od_vae(state_dict: Dict, vae: tf.keras.Model) -> OutlierVAE:
    """
    Initialize OutlierVAE.
    ...
    """
    od = OutlierVAE(
        threshold=state_dict["threshold"], score_type=state_dict["score_type"], vae=vae, samples=state_dict["samples"]
    )
    return od


def init_ad_ae(
    state_dict: Dict, ae: tf.keras.Model, model: tf.keras.Model, model_hl: List[tf.keras.Model]
) -> AdversarialAE:
    """
...
def init_ad_md(state_dict: Dict, distilled_model: tf.keras.Model, model: tf.keras.Model) -> ModelDistillation:
    """
...
def init_od_s2s(state_dict: Dict, seq2seq: tf.keras.Model) -> OutlierSeq2Seq:
    """
...
def load_text_embed(
    filepath: Union[str, os.PathLike], load_dir: str = "model"
...
def init_preprocess(
    ...
    emb: Optional[TransformerEmbedding],
    ...
    **kwargs,
    ...
    if kwargs:  # override defaults
        ...
        model is not None
        and callable(state_dict["preprocess_fn"])
...
def init_cd_classifierdrift(
    clf_drift: tf.keras.Model,
    state_dict: Dict,
    ...
    **kwargs,
    ...
    state_dict["kwargs"]["train_kwargs"]["optimizer"] = tf.keras.optimizers.get(
