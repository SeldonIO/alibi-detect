...
def test_save_ksdrift(data, preprocess_fn, tmp_path):
    ...
    np.testing.assert_array_equal(cd.predict(X_h0)["data"]["p_val"], cd_load.predict(X_h0)["data"]["p_val"])
...
@pytest.mark.skipif(version.parse(scipy.__version__) < version.parse("1.7.0"), reason="Requires scipy version >= 1.7.0")
@parametrize_with_cases("data", cases=ContinuousData, prefix="data_")
...
def test_save_classifierdrift(data, optimizer, classifier_model, backend, tmp_path, seed):  # noqa: F811
    ...
    with fixed_seed(seed):
        cd = ClassifierDrift(
            X_ref, model=classifier_model, p_val=P_VAL, optimizer=optimizer, n_folds=5, backend=backend, train_size=None
        )
...
def test_save_spotthediff(data, classifier_model, backend, tmp_path, seed):  # noqa: F811
    ...
    X_ref, X_h0 = data
    with fixed_seed(seed):
        cd = SpotTheDiffDrift(X_ref, p_val=P_VAL, n_folds=5, train_size=None, backend=backend)
        preds = cd.predict(X_h0)  # noqa: F841
    save_detector(cd, tmp_path)
...
@parametrize("deep_kernel", [{"kernel_a": "rbf", "eps": 0.01}], indirect=True)  # Default for kernel_a
@parametrize_with_cases("data", cases=ContinuousData, prefix="data_")
def test_save_learnedkernel(data, deep_kernel, backend, tmp_path, seed):  # noqa: F811
    ...
    X_ref, X_h0 = data
    with fixed_seed(seed):
        cd = LearnedKernelDrift(X_ref, deep_kernel, p_val=P_VAL, backend=backend, train_size=0.7, num_workers=0)
        preds = cd.predict(X_h0)  # noqa: F841
    save_detector(cd, tmp_path)
    with fixed_seed(seed):
        cd_load = load_detector(tmp_path)
...
def test_save_classifieruncertaintydrift(data, classifier_model, backend, tmp_path, seed):  # noqa: F811
    ...
    with fixed_seed(seed):
        cd = ClassifierUncertaintyDrift(
            X_ref, model=classifier_model, p_val=P_VAL, backend=backend, preds_type="probs", uncertainty_type="entropy"
        )
        preds = cd.predict(X_h0)  # noqa: F841
    save_detector(cd, tmp_path)
...
def test_save_regressoruncertaintydrift(data, regressor, backend, tmp_path, seed):
    ...
    with fixed_seed(seed):
        cd = RegressorUncertaintyDrift(
            X_ref, model=regressor, p_val=P_VAL, backend=backend, uncertainty_type="mc_dropout"
        )
        preds = cd.predict(X_h0)  # noqa: F841
    save_detector(cd, tmp_path)
...
def test_save_onlinecvmdrift(data, preprocess_custom, tmp_path, seed):
    ...
    with fixed_seed(seed):
        cd = CVMDriftOnline(
            X_ref, ert=ERT, preprocess_fn=preprocess_custom, n_bootstraps=N_BOOTSTRAPS, window_sizes=[WINDOW_SIZE]
        )
        stats = []
        for i, x_t in enumerate(X_h0):
            pred = cd.predict(x_t)
...
def test_save_onlinefetdrift(data, tmp_path, seed):
    ...
    with fixed_seed(seed):
        cd = FETDriftOnline(X_ref, ert=ERT, n_bootstraps=N_BOOTSTRAPS, window_sizes=[WINDOW_SIZE])
        stats = []
        for i, x_t in enumerate(X_h0):
            pred = cd.predict(x_t)
            if i >= WINDOW_SIZE:  # test stats garbage until at least one window full
...
def test_save_preprocess_nlp(data, preprocess_fn, tmp_path, backend):
    ...
    # Save preprocess_fn to config
    filepath = tmp_path
    cfg_preprocess = _save_preprocess_config(
        preprocess_fn, input_shape=(768,), filepath=filepath  # hardcoded to bert-base-cased for now
    )
    cfg_preprocess = _path2str(cfg_preprocess)
    cfg_preprocess = PreprocessConfig(**cfg_preprocess).dict()  # pydantic validation
    assert cfg_preprocess["src"] == "@cd." + registry_str + ".preprocess.preprocess_drift"
...
def test_replace():
    ...
    dict1 = {
        "key1": "key1",
        "key7": None,
        "dict2": {"key2": "key2", "key4": None, "dict3": {"key5": "key5", "key6": None}},
    }
