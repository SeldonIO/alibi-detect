...
def encoder_model(backend, current_cases):
    ...
    if backend == "tensorflow":
        ...
        model = nn.Sequential(nn.Linear(input_dim, 5), nn.ReLU(), nn.Linear(5, LATENT_DIM))
    else:
...
def encoder_dropout_model(backend, current_cases):
    ...
    if backend == "tensorflow":
        ...
        model = nn.Sequential(
            nn.Linear(input_dim, 5), nn.ReLU(), nn.Dropout(0.0), nn.Linear(5, LATENT_DIM)  # 0.0 to ensure determinism
        )
    else:
...
def classifier_model(backend, current_cases):
    ...
    if backend == "tensorflow":
        ...
        model = nn.Sequential(nn.Linear(input_dim, 2), nn.Softmax(1))
...
def nlp_embedding_and_tokenizer(model_name, max_len, uae, backend):
    ...
    tokens = tokenizer(list(X[:5]), pad_to_max_length=True, max_length=max_len, return_tensors=backend)
