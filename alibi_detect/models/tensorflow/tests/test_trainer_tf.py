...
tests = list(product(X_train, y_train, dataset, loss_fn_kwargs, verbose))
n_tests = len(tests)


@pytest.fixture
def trainer_params(request):
    x_train, y_train, dataset, loss_fn_kwargs, verbose = tests[request.param]
    return x_train, y_train, dataset, loss_fn_kwargs, verbose


@pytest.mark.parametrize('trainer_params', list(range(n_tests)), indirect=True)
def test_trainer(trainer_params):
    x_train, y_train, dataset, loss_fn_kwargs, verbose = trainer_params
    ...
    trainer(model, categorical_crossentropy, x_train, y_train=y_train, dataset=ds,
            loss_fn_kwargs=loss_fn_kwargs, epochs=2, verbose=verbose)
