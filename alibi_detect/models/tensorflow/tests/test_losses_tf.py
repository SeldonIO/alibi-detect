...
def test_elbo():
    ...
    assert elbo(x, y, sim=0.05).numpy() > 0
    assert elbo(x, x, sim=0.05).numpy() < 0
...
def test_loss_aegmm():
    loss = loss_aegmm(x, y, z, gamma, w_energy=0.1, w_cov_diag=0.005)
    loss_no_cov = loss_aegmm(x, y, z, gamma, w_energy=0.1, w_cov_diag=0.0)
    loss_xx = loss_aegmm(x, x, z, gamma, w_energy=0.1, w_cov_diag=0.0)
    assert loss > loss_no_cov
    assert loss_no_cov > loss_xx


def test_loss_vaegmm():
    loss = loss_vaegmm(x, y, z, gamma, w_recon=1e-7, w_energy=0.1, w_cov_diag=0.005)
    loss_no_recon = loss_vaegmm(x, y, z, gamma, w_recon=0.0, w_energy=0.1, w_cov_diag=0.005)
    loss_no_recon_cov = loss_vaegmm(x, y, z, gamma, w_recon=0.0, w_energy=0.1, w_cov_diag=0.0)
    loss_xx = loss_vaegmm(x, x, z, gamma, w_recon=1e-7, w_energy=0.1, w_cov_diag=0.005)
...
def test_loss_adv_ae():
    loss = loss_adv_ae(x, y, model, w_model=1.0, w_recon=0.0)
    loss_with_recon = loss_adv_ae(x, y, model, w_model=1.0, w_recon=1.0)
    assert loss > 0.0
    assert loss_with_recon > loss


layers = [tf.keras.layers.InputLayer(input_shape=(x.shape[1],)), tf.keras.layers.Dense(5, activation=tf.nn.softmax)]
