...
class PixelCNN(distribution.Distribution):
    def __init__(self,
        ...
        num_hierarchies
            The number of hightest-level blocks (separated by expansions/contractions of dimensions in Figure 2 of [1].)
        num_filters
    ...
    def _make_mixture_dist(self, component_logits, locs, scales, return_per_pixel=False):
    ...
    def _log_prob(self, value, conditional_input=None, training=None, return_per_pixel=False):
        ...
        dist = self._make_mixture_dist(component_logits, locs, scales, return_per_pixel=return_per_pixel)
...
class _PixelCNNNetwork(tf.keras.layers.Layer):
  ...
  def build(self, input_shape):
    ...
    kernel_valid_dims = {'vertical': (rows - 1, cols),  # vertical stack
                         'horizontal': (2, cols // 2 + 1)}  # horizontal stack TODO: why 2 rows?
    ...
    for i in range(self._num_hierarchies):
      for _ in range(self._num_resnet):
        ...
        for stack in ['vertical', 'horizontal']:
          ...
          if stack == 'horizontal':
            h = activation(layer_stacks['vertical'][-1])  # TODO: not really like paper... b/c from within stack
          ...
          x = _apply_sigmoid_gating(x)  # TODO: no tanh?
    ...
    # block.
    for i in range(self._num_hierarchies):
      num_resnet = self._num_resnet if i == 0 else self._num_resnet + 1  # TODO: why +1?
      ...
      for _ in range(num_resnet):
        ...
        for stack in ['vertical', 'horizontal']:
          ...
          if stack == 'horizontal':
            x_symmetric = tf.keras.layers.Concatenate(axis=-1, dtype=dtype)([upward_pass['vertical'], x_symmetric])
