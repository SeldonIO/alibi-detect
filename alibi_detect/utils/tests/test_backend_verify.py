...
class TestBackendValidator:
    ...
    def test_backend_verifier_error_msg(self):
        ...
        assert str(err.value) == (
            "test backend not implemented. Use one of `backend_1`, " "`backend_2` and `backend_3` instead."
        )
        ...
        assert str(err.value) == (
            "`backend_B` not installed. Cannot initialize and run test with backend_1 backend."
        ...
        assert (
            "`backend_D` and `backend_E` not installed. Cannot initialize and run test with backend_3 backend. "
            "The necessary missing dependencies can be installed using `pip install "
            "alibi-detect[backend_D backend_E]`."
        ) == str(err.value)
