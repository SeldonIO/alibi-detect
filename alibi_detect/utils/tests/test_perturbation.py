...
def test_apply_mask(apply_mask_params):
    ...
    X_mask, mask = apply_mask(
        x, mask_size=mask_size, n_masks=n_masks, channels=channels, mask_type=mask_type, clip_rng=clip_rng
    )
    ...
    if mask_type == "zero":
        ...
        assert (X_mask == np.clip(x + mask, clip_rng[0], clip_rng[1])).astype(int).sum() == reduce(
            mul, list(x.shape)
        ) * n_masks
...
def test_inject_outlier_ts(inject_outlier_ts_params):
    ...
    data = inject_outlier_ts(X, perc_outlier, perc_window=10, n_std=2.0, min_std=min_std)
