...
def test_validate_config(select_cfg):
    ...
    # Check returned cfg doesn't contain version or config_spec
    assert 'version' not in cfg_full
    assert 'config_spec' not in cfg_full
    # Check values of items in cfg unchanged
    for k, v in cfg.items():
        if k != 'version' and k != 'config_spec':
    ...
    # Check warning raised if version or config_spec different
    cfg_err = cfg.copy()
    cfg_err['version'] = '0.1.x'
    with pytest.warns(Warning):  # error will be raised if a warning IS NOT raised
        _ = validate_config(cfg_err, resolved=True)
    cfg_err = cfg.copy()
    cfg_err['config_spec'] = '0.1.x'
    with pytest.warns(Warning):  # error will be raised if a warning IS NOT raised
        _ = validate_config(cfg_err, resolved=True)
