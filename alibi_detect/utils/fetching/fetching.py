...
def get_pixelcnn_default_kwargs():
    ...
    KWARGS_PIXELCNN = {"dist_s": dist, "dist_b": dist.copy(), "input_shape": (28, 28, 1)}
...
def fetch_enc_dec(url: str, filepath: Union[str, os.PathLike]) -> None:
    ...
    # encoder and decoder
    tf.keras.utils.get_file(model_path.joinpath("encoder_net.h5"), _join_url(url_models, "encoder_net.h5"))
    tf.keras.utils.get_file(model_path.joinpath("decoder_net.h5"), _join_url(url_models, "decoder_net.h5"))
...
def fetch_ae(url: str, filepath: Union[str, os.PathLike]) -> None:
    ...
    model_path = Path(filepath).joinpath("model").resolve()
    # encoder and decoder
    tf.keras.utils.get_file(model_path.joinpath("checkpoint"), _join_url(url_models, "checkpoint"))
    tf.keras.utils.get_file(model_path.joinpath("ae.ckpt.index"), _join_url(url_models, "ae.ckpt.index"))
    tf.keras.utils.get_file(
        model_path.joinpath("ae.ckpt.data-00000-of-00001"), _join_url(url_models, "ae.ckpt.data-00000-of-00001")
    )
...
def fetch_ad_ae(url: str, filepath: Union[str, os.PathLike], state_dict: dict) -> None:
    ...
    url_models = _join_url(url, "model")
    model_path = Path(filepath).joinpath("model").resolve()
    tf.keras.utils.get_file(model_path.joinpath("model.h5"), _join_url(url_models, "model.h5"))
    tf.keras.utils.get_file(model_path.joinpath("checkpoint"), _join_url(url_models, "checkpoint"))
    tf.keras.utils.get_file(model_path.joinpath("ae.ckpt.index"), _join_url(url_models, "ae.ckpt.index"))
    tf.keras.utils.get_file(
        model_path.joinpath("ae.ckpt.data-00000-of-00002"), _join_url(url_models, "ae.ckpt.data-00000-of-00002")
    )
    tf.keras.utils.get_file(
        model_path.joinpath("ae.ckpt.data-00001-of-00002"), _join_url(url_models, "ae.ckpt.data-00001-of-00002")
    )
    hidden_layer_kld = state_dict["hidden_layer_kld"]
    if hidden_layer_kld:
        for i, (_, _) in enumerate(hidden_layer_kld.items()):
            hl = "model_hl_" + str(i)
            tf.keras.utils.get_file(model_path.joinpath(hl + ".ckpt.index"), _join_url(url_models, hl + ".ckpt.index"))
            tf.keras.utils.get_file(
...
def fetch_ad_md(url: str, filepath: Union[str, os.PathLike]) -> None:
    ...
    # encoder and decoder
    tf.keras.utils.get_file(model_path.joinpath("model.h5"), _join_url(url_models, "model.h5"))
    tf.keras.utils.get_file(model_path.joinpath("distilled_model.h5"), _join_url(url_models, "distilled_model.h5"))
...
def fetch_aegmm(url: str, filepath: Union[str, os.PathLike]) -> None:
    ...
    url_models = _join_url(url, "model")
    model_path = Path(filepath).joinpath("model").resolve()
    tf.keras.utils.get_file(model_path.joinpath("gmm_density_net.h5"), _join_url(url_models, "gmm_density_net.h5"))
    tf.keras.utils.get_file(model_path.joinpath("checkpoint"), _join_url(url_models, "checkpoint"))
    tf.keras.utils.get_file(model_path.joinpath("aegmm.ckpt.index"), _join_url(url_models, "aegmm.ckpt.index"))
    tf.keras.utils.get_file(
        model_path.joinpath("aegmm.ckpt.data-00000-of-00001"), _join_url(url_models, "aegmm.ckpt.data-00000-of-00001")
    )
...
def fetch_vae(url: str, filepath: Union[str, os.PathLike]) -> None:
    ...
    model_path = Path(filepath).joinpath("model").resolve()
    tf.keras.utils.get_file(model_path.joinpath("checkpoint"), _join_url(url_models, "checkpoint"))
    tf.keras.utils.get_file(model_path.joinpath("vae.ckpt.index"), _join_url(url_models, "vae.ckpt.index"))
    tf.keras.utils.get_file(
        model_path.joinpath("vae.ckpt.data-00000-of-00001"), _join_url(url_models, "vae.ckpt.data-00000-of-00001")
    )
...
def fetch_vaegmm(url: str, filepath: Union[str, os.PathLike]) -> None:
    ...
    tf.keras.utils.get_file(model_path.joinpath("gmm_density_net.h5"), _join_url(url_models, "gmm_density_net.h5"))
    tf.keras.utils.get_file(model_path.joinpath("checkpoint"), _join_url(url_models, "checkpoint"))
    tf.keras.utils.get_file(model_path.joinpath("vaegmm.ckpt.index"), _join_url(url_models, "vaegmm.ckpt.index"))
    tf.keras.utils.get_file(
        model_path.joinpath("vaegmm.ckpt.data-00000-of-00001"), _join_url(url_models, "vaegmm.ckpt.data-00000-of-00001")
    )
...
def fetch_seq2seq(url: str, filepath: Union[str, os.PathLike]) -> None:
    ...
    # save seq2seq
    tf.keras.utils.get_file(model_path.joinpath("checkpoint"), _join_url(url_models, "checkpoint"))
    tf.keras.utils.get_file(model_path.joinpath("seq2seq.ckpt.index"), _join_url(url_models, "seq2seq.ckpt.index"))
    tf.keras.utils.get_file(
    ...
    tf.keras.utils.get_file(model_path.joinpath("threshold_net.h5"), _join_url(url_models, "threshold_net.h5"))
...
def fetch_llr(url: str, filepath: Union[str, os.PathLike]) -> str:
    ...
    if not model_path.is_dir():
        model_path.mkdir(parents=True, exist_ok=True)
    try:
        tf.keras.utils.get_file(model_path.joinpath("model_s.h5"), _join_url(url_models, "model_s.h5"))
        tf.keras.utils.get_file(model_path.joinpath("model_b.h5"), _join_url(url_models, "model_b.h5"))
        model_type = "weights"
        return model_type
    except Exception:
        tf.keras.utils.get_file(model_path.joinpath("model.h5"), _join_url(url_models, "model.h5"))
        tf.keras.utils.get_file(
            model_path.joinpath("model_background.h5"), _join_url(url_models, "model_background.h5")
        )
        return "model"


def fetch_state_dict(url: str, filepath: Union[str, os.PathLike], save_state_dict: bool = True) -> Tuple[dict, dict]:
    """
...
def fetch_detector(
    filepath: Union[str, os.PathLike], detector_type: str, dataset: str, detector_name: str, model: str = None
) -> Data:
    """
