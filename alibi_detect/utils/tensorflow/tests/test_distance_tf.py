...
def test_bckm(bckm_params):
    ...
    kernel = GaussianRBF(sigma=tf.constant(1.0))
...
@pytest.mark.parametrize("mmd_from_kernel_matrix_params", list(range(n_tests_mmd_from_kernel_matrix)), indirect=True)
...
def test_relative_euclidean_distance():
    ...
    assert (relative_euclidean_distance(x, y).numpy() >= 0.0).all()
...
@pytest.mark.parametrize("permed_lsdds_params", list(range(n_tests_permed_lsdds)), indirect=True)
def test_permed_lsdds(permed_lsdds_params):
    ...
    H = GaussianRBF(np.sqrt(2.0) * sigma)(kcs, kcs)

    perms = [tf.random.shuffle(tf.range(n + m)) for _ in range(B)]
    x_perms = [perm[:n] for perm in perms]
    y_perms = [perm[n:] for perm in perms]

    lsdd_perms, H_lam_inv, lsdd_unpermed = permed_lsdds(k_all_c, x_perms, y_perms, H, return_unpermed=True)
