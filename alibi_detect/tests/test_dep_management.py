...
def check_correct_dependencies(module: ModuleType, dependencies: defaultdict, opt_dep: str):
...
def test_ad_dependencies(opt_dep):
    ...
    dependency_map = defaultdict(lambda: ["default"])
    for dependency, relations in [("AdversarialAE", ["tensorflow"]), ("ModelDistillation", ["tensorflow"])]:
        dependency_map[dependency] = relations
...
def test_fetching_utils_dependencies(opt_dep):
    ...
    dependency_map = defaultdict(lambda: ["default"])
    for dependency, relations in [("fetch_detector", ["tensorflow"]), ("fetch_tf_model", ["tensorflow"])]:
        dependency_map[dependency] = relations
