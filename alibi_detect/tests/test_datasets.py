...
def test_fetch_ecg(return_X_y):
    ...
    if return_X_y:
        assert isinstance(data, tuple)
        assert (
            isinstance(data[0][0], np.ndarray)
            and isinstance(data[0][1], np.ndarray)
            and isinstance(data[1][0], np.ndarray)
            and isinstance(data[1][1], np.ndarray)
        )
    else:
        assert isinstance(data, Bunch)
        assert (
            isinstance(data.data_train, np.ndarray)
            and isinstance(data.data_test, np.ndarray)
            and isinstance(data.target_train, np.ndarray)
            and isinstance(data.target_test, np.ndarray)
        )
