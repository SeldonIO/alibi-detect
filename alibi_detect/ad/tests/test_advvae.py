...
tests = list(product(threshold, samples, beta, w_model, w_recon, threshold_perc, return_instance_score))
n_tests = len(tests)
...
@pytest.fixture
def adv_vae_params(request):
    return tests[request.param]


@pytest.mark.parametrize('adv_vae_params', list(range(n_tests)), indirect=True)
def test_adv_vae(adv_vae_params):
    # AdversarialVAE parameters
    threshold, samples, beta, w_model, w_recon, threshold_perc, return_instance_score = adv_vae_params
    ...
    assert advvae.threshold == threshold
    assert advvae.meta == {'name': 'AdversarialVAE', 'detector_type': 'offline', 'data_type': None}
    for layer in advvae.model.layers:
        assert not layer.trainable

    # fit AdversarialVAE, infer threshold and compute scores
    advvae.fit(X, w_model=w_model, w_recon=w_recon, epochs=5, verbose=False)
