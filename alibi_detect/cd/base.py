...
class BaseUnivariateDrift(BaseDetector):
    ...
    def predict(self, X: Union[np.ndarray, list], drift_type: str = 'batch',
        ...
        # TODO: (Arnaud) should `score` take Union[np.ndarray, list] ?
        #  Also, we should document exactly when a list is expected (i.e. text)
        p_vals, dist = self.score(X)  # type: ignore
        ...
        if drift_type == 'feature':
            ...
            drift_pred, threshold = fdr(p_vals, q_val=self.p_val)  # type: ignore
        else:
