...
class BaseNMEDrift(BaseDetector):
    def __init__(
            self,
            x_ref: np.ndarray,
            p_val: float = .05,
        ...
        """
        Base class for the NME drift detector.
        ...
        self.meta['detector_type'] = 'offline'
        self.meta['data_type'] = data_type
        self.meta['params'] = {'n_test_locations': n_test_locations}
    ...
    def init_test_locations(self, x_ref: np.ndarray) -> np.ndarray:
        # test_inds = np.random.choice(np.arange(self.n), self.J, replace=False)
        # return x_ref[test_inds]
        return np.zeros(shape=(self.J,) + x_ref.shape[1:]).astype(x_ref.dtype) + 0.5
    ...
    def get_splits(self, x_ref: np.ndarray, x: np.ndarray) \
        ...
        x_ref_shuffled = x_ref[np.random.permutation(x_ref.shape[0])]
        x_shuffled = x[np.random.permutation(x.shape[0])]
        n_tot = min(len(x_ref), len(x))  # Results in subsampling of larger window
        n_tr = int(n_tot * self.train_size)
        x_ref_train, x_ref_test = x_ref_shuffled[:n_tr], x_ref_shuffled[n_tr:n_tot]
        x_train, x_test = x_shuffled[:n_tr], x_shuffled[n_tr:n_tot]

        return (x_ref_train, x_ref_test), (x_train, x_test)
    ...
    def predict(self, x: np.ndarray,  return_p_val: bool = True,
        ...
        """
        # compute drift scores
        p_val, dist, test_locs = self.score(x)
        drift_pred = int(p_val < self.p_val)
