...
tests_ksdrift = list(product(n_features, n_enc, preprocess, alternative, correction, update_x_ref, preprocess_at_init))
...
def test_ksdrift(ksdrift_params):
    n_features, n_enc, preprocess, alternative, correction, update_x_ref, preprocess_at_init = ksdrift_params
    np.random.seed(0)
