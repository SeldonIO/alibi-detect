...
def test_classifier_uncertainty(cu_params):
    ...
    uncertainties = classifier_uncertainty(X_test, model_fn, preds_type=preds_type, uncertainty_type=uncertainty_type)
...
def test_regressor_uncertainty(ru_params):
    ...
    if uncertainty_type == "dropout":
        model = nn.Sequential(nn.Linear(n_features, 10), nn.Dropout(0.5), nn.Linear(10, 1))
    else:
    ...
    uncertainties = regressor_uncertainty(X_test, model_fn, uncertainty_type=uncertainty_type)
    assert uncertainties.shape == (X_test.shape[0], 1)
