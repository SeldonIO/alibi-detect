...
class _Encoder(tf.keras.Model):
    def __init__(
        ...
        if isinstance(mlp, tf.keras.Model):
            ...
            raise ValueError(
                "Need to provide either `enc_dim` and `step_dim` or a " "tf.keras.Sequential or tf.keras.Model `mlp`"
            )
...
class UAE(tf.keras.Model):
    def __init__(
        ...
        if is_enc:
            ...
            raise ValueError(
                "Need to provide either `enc_dim` or a tf.keras.Sequential" " or tf.keras.Model `encoder_net`."
            )
...
class HiddenOutput(tf.keras.Model):
    def __init__(
        self, model: tf.keras.Model, layer: int = -1, input_shape: tuple = None, flatten: bool = False
    ) -> None:
        super().__init__()
