...
@pytest.mark.parametrize(
    "model, use_calibration, calibration_kwargs",
    [
...
def test_predict_proba1(model, preds_type, use_calibration, binarize_preds):
    ...
    """
    drift_detector = ClassifierDriftSklearn(
        x_ref=np.random.randn(100, 5),
        model=model,
        preds_type=preds_type,
        use_calibration=use_calibration,
    ...
    np.testing.assert_allclose(internal_model.predict(x_te), internal_model.aux_predict_proba(x_te)[:, 1])


@pytest.mark.parametrize(
    "model", [LogisticRegression(), GradientBoostingClassifier(), AdaBoostClassifier(), QuadraticDiscriminantAnalysis()]
)
...
def test_predict_proba2(model, pred_types, use_calibration, binarize_preds):
    ...
    # check if predict matches the new predict_proba
    np.testing.assert_allclose(internal_model.decision_function(x_te), internal_model.aux_predict_proba(x_te)[:, 1])
...
@pytest.mark.parametrize("preds_type", ["probs"])
@pytest.mark.parametrize(
    "binarize_preds, use_calibration, use_oob",
    [
        (False, False, False),
        (False, False, True),
        (False, True, False),
...
@pytest.mark.parametrize("model", [LogisticRegression(), GradientBoostingClassifier(), AdaBoostClassifier()])
@pytest.mark.parametrize("preds_type", ["probs"])
...
def test_clone_rf1(model, preds_type, use_oob):
    ...
    with pytest.raises(ValueError):
        ClassifierDriftSklearn(x_ref=np.random.randn(100, 5), model=model, preds_type=preds_type, use_oob=use_oob)
...
@pytest.mark.parametrize("model", [RandomForestClassifier(oob_score=True), RandomForestClassifier(oob_score=False)])
@pytest.mark.parametrize("preds_type", ["probs"])
...
def test_clone_rf3(model, preds_type, use_oob):
    ...
    cd = ClassifierDriftSklearn(x_ref=np.random.randn(100, 5), model=model, preds_type=preds_type, use_oob=use_oob)
