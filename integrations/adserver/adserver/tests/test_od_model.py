...
class DummyODModel(BaseDetector):
    def __init__(self, expected_return_instance_score: bool= True, expected_return_feature_score: bool = True, expect_return_is_outlier: int = 0):
        super().__init__()
...
class TestAEModel(TestCase):
    ...
    def test_no_return_instance_score(self):
        ...
        model = DummyODModel(expected_return_instance_score=False, expect_return_is_outlier=expect_return_is_outlier)
        ad_model = AlibiDetectAdversarialDetectionModel(
            "name", "s3://model", model=model
        )
        req = [1, 2]
