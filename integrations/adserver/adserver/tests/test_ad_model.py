...
class DummyAEModel(BaseDetector):
    def __init__(self, expected_return_instance_score: bool= True, expected_is_adversarial:int = 1):
        super().__init__()
...
class TestAEModel(TestCase):
    ...
    def test_no_return_instance_score(self):
        ...
        model = DummyAEModel(expected_return_instance_score=False,expected_is_adversarial=expected_is_adversarial)
        ad_model = AlibiDetectAdversarialDetectionModel(
            "name", "s3://model", model=model
        )
        req = [1, 2]
