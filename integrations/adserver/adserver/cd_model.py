...
from adserver.model import AlibiDetectModel


class AlibiDetectConceptDriftModel(AlibiDetectModel):  # pylint:disable=c-extension-no-member
    def __init__(self, name: str, storage_uri: str, model: Optional[Data] = None, drift_batch_size: int = 1000):
        """
    ...
    def process_event(self, inputs: List, headers: Dict) -> Dict:
        ...
        if self.batch.shape[0] >= self.drift_batch_size:
            logging.info("Running drift detection. Batch size is %d. Needed %d",self.batch.shape[0], self.drift_batch_size)
            cd_preds = self.model.predict(X)
            self.batch = None
            return json.loads(json.dumps(cd_preds, cls=NumpyEncoder))
        else:
            logging.info("Not running drift detection. Batch size is %d. Need %d",self.batch.shape[0], self.drift_batch_size)
